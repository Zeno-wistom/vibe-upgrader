...
一个用于升级 UI/UX、视觉层级、操作路径、交互反馈与动效语言的 Agent Skill。
...
[在线 Showcase](https://vibe-upgrader-showcase.vercel.app/) · [AIGC 真实案例](https://vibe-upgrader-aigc-case.vercel.app/)
  ...
  <tr>
    <td width="50%" align="center">
      <a href="https://vibe-upgrader-showcase.vercel.app/">
        <img src="./docs/media/showcase_readme_cover.png?v=20260724" alt="Vibe-Upgrader 交互式 Showcase">
      </a>
      <br>
...
| 你关心的问题 | 答案 |
| --- | --- |
| 这是什么？ | 一个遵循 Agent Skills 开放格式、需要显式调用的可移植 Skill，不是网页模板或组件包。 |
| 它能升级什么？ | 信息层级、操作路径、状态反馈、动效语言、响应式体验与品牌表达。 |
| 支持哪些 Agent？ | Codex、Claude Code，以及其他实现 Agent Skills 标准或能够加载 `SKILL.md` 的客户端。 |
...
Vibe-Upgrader 的核心遵循 [Agent Skills 开放格式](https://agentskills.io/specification)。不同客户端使用不同目录：
...
其他客户端请使用其 Agent Skills 目录和调用语法。格式兼容不代表每个 Agent 都具备相同的浏览器、组件检索或文件操作能力。
...
推荐按这五项描述，信息越具体，结果越稳定。下面以 Codex 为例；Claude Code 将首行改为 `/vibe-upgrader`：
...
![Vibe-Upgrader 四步工作流程](./docs/media/workflow-overview-zh.svg?v=20260724)
...
![拖动查看升级前后差异](./docs/media/showcase_interaction.gif?v=20260724)

<details>
<summary>查看桌面端与移动端静态截图</summary>

![Showcase 桌面端](./docs/media/showcase_desktop.png?v=20260724)

![Showcase 移动端](./docs/media/showcase_mobile.png?v=20260724)
...
├── agents/           # Codex / OpenAI 可选元数据；核心入口仍是 SKILL.md
...
- 核心目录遵循 [Agent Skills 规范](https://agentskills.io/specification)，主入口是标准 `SKILL.md`。
- [Codex](https://developers.openai.com/codex/skills) 使用 `.agents/skills`，支持 `$vibe-upgrader` 显式调用。
- [Claude Code](https://code.claude.com/docs/en/skills) 使用 `.claude/skills`，支持 `/vibe-upgrader` 显式调用。
- 其他实现 Agent Skills 标准或能够加载 `SKILL.md` 的客户端可以复用核心 Skill；具体工具能力和调用语法由客户端决定。
- 核心说明文件不需要 Node.js；可选辅助脚本和验证工具需要 Python **3.10+**。
